...
module.exports = {
  placeOrder: async (req, res, next) => {
    ...
    if (paymentInfo.paymentType === "CARD") {
      ...
      if (
        !orderStripeCharge.id ||
        !orderStripeCharge.status === "succeeded" ||
        !orderStripeCharge.status === "pending"
      ) {
        return res
          .status(500)
          .send({ success: false, error: "Error chargin customer" });
      }

      const { brand, exp_year, exp_month, last4 } =
        orderStripeCharge.payment_method_details.card;
      ...
      paymentInfo = {
        ...
        paymentStatus:
          orderStripeCharge.status === "succeeded"
            ? "COMPLETE"
            : orderStripeCharge.status === "failed"
            ? "FAILED"
            : orderStripeCharge.status === "pending"
            ? "ON-HOLD"
            : null,
  ...
  //   console.log({ updateOrder: { status, orderId } });

  //   if (
  //     !status ||
  //     !["ONGOING", "SHIPPED", "COMPLETE", "CANCELLED"].includes(status)
  //   ) {
  //     return res
  //       .status(500)
  //       .send({
  //         success: false,
  //         error: `Invalid order status sent => ${status}`,
  //       });
  //   }
  ...
  //       return res
  //         .status(500)
  //         .send({ success: false, error: `Error updating order: ${err}` });
  //     }
